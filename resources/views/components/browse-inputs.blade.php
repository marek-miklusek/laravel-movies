        ...
        <div id="dropdown" class="z-10 hidden divide-y divide-gray-100 rounded-lg w-44 bg-[#333]">
            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                @foreach ($languages as $key => $lang)
                    <li>
                        <a href="{{ route('browse.lang', $key) }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                            {{ $lang }}
                        </a>
                    </li>
                @endforeach
                ...
                @foreach ($genres['name'] as $name)
                    <li>
                        <a href="{{ route('browse.genre', [$name, $genres['route_lang'], $genres['route_sort'] ?? '']) }}" 
                            class="capitalize block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                            {{ $name }}
                        </a>
                    </li>
                @endforeach
